...
def summarize(
    label: str, intervals: list[tuple[float, float]], warmup: int = 2
) -> dict:
...
def write_raw(
    fps_req: float, imposed: list, effective: list, path: Path
) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open('w', newline='') as f:
        writer = csv.writer(f)
...
def plot(rows: list[dict], path: Path) -> None:
    ...
    ax.plot(
        [lim_lo, lim_hi], [lim_lo, lim_hi], 'k--', alpha=0.3, label='y = x'
    )
    ...
    ax_err.plot(req, err_req, 'o-', color='C2', label='requested - effective')
    ax_err.plot(req, err_imp, 's-', color='C3', label='imposed - effective')
